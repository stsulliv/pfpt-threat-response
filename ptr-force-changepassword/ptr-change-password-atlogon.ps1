...
function Get-PtrKey ($PathRoot) {
    ...
    if (!([System.IO.File]::Exists("$PathRoot\threatresponse.cred"))) {
        ...
        $ptr_key = Read-Host "Enter your API Key from Threat Response"
        $ptr_key | ConvertTo-SecureString -AsPlainText -Force | ConvertFrom-SecureString | Out-File "$PathRoot\threatresponse.cred" -Force

        return $ptr_key
    }
    else {
        $ptr_keySec = Get-Content "$PathRoot\threatresponse.cred" | ConvertTo-SecureString
        $ptr_key = [Runtime.InteropServices.Marshal]::PtrToStringAuto([Runtime.InteropServices.Marshal]::SecureStringToBSTR((($ptr_keySec))))
